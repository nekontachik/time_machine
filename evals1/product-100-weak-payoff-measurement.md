...
**Суддя позначив 2 нудні кінцівки зі 100 → 2.0%** (95% CI [0.6%, 7.0%]).

> ⚠ Це **оцінка судді**, а не ground truth. Суддя `gemini-3.1-flash-lite` валідований (precision 92.9%, recall 100% проти людського gold). Обидва fail-кандидати потребують твого вердикту (картки нижче) — при 2 прапорцях і precision 92.9% один із них статистично може бути хибною тривогою.
...
Не факт — суддя не ідеальний. Винеси вердикт по кожному.
...
- **Оцінка, не ground truth.** 2 fail — вердикт судді; підтверди картки, перш ніж рахувати це фактом.
