...
- **AI Images**: fal.ai → Flux 1 Schnell (`lib/openai.ts`, `buildFluxPrompt`)
...
2. Files: lib/openai.ts, lib/claude.ts, lib/redis.ts, lib/rateLimit.ts,
...
## Key Lib Files
- `lib/openai.ts` — fal.ai Flux image wrapper, exports `buildFluxPrompt`
- `lib/claude.ts` — OpenRouter text generation (Gemini for events, Claude for scenarios)
- `lib/rateLimit.ts` — Redis key `ratelimit:{ip}:{YYYY-MM-DD}`, TTL 24h; limit = `RATE_LIMIT_FREE` env (default 3)
- `lib/redis.ts` — ioredis client for event caching (24h TTL)
